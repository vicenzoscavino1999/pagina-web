...
export class ContentHydrator {
    ...
    #hydrateFlow(): void {
        ...
        const connectorHtml = '<div class="hidden md:block absolute top-12 left-[16%] right-[16%] h-0.5 bg-slate-100 z-0"></div>';
        const stepsHtml = this.#content.flow.steps
            .map((step, index) => {
                const delay = index > 0 ? ` style="transition-delay: ${index * 100}ms;"` : '';
                return `
                    <div class="reveal relative z-10"${delay}>
                        <div class="${step.iconWrapperClass}">
